...
When `{{discovery.platform_capabilities.hook_mechanism}} == "typescript_plugin"` (i.e. OpenCode), apply these adjustments when generating command files.

### Frontmatter Differences

OpenCode commands use different frontmatter fields:

| Claude Code | OpenCode | Notes |
|-------------|----------|-------|
| `argument-hint: [hint]` | *(not supported)* | Omit entirely. Describe arguments in the instruction body instead. |
| `disallowed-tools: Task, EnterPlanMode` | `agent: planner` | OpenCode restricts tools via agent definitions, not per-command denylists. Create an agent with `tools: { task: false }` and reference it. |
| `model: {{discovery.models.complex}}` | `model: {{discovery.models.complex}}` | Same field, same usage. |
| `description: ...` | `description: ...` | Same field, same usage. |
| *(n/a)* | `subtask: true` | Set on commands that should run as subtasks (e.g. `/dev`). |

Example OpenCode command frontmatter:
...
Where `planner` references an agent defined in `.opencode/agents/planner.md` with appropriate tool restrictions.

### Tool Name Mapping

When `{{discovery.platform_capabilities.tool_name_style}} == "lowercase"`, replace all tool name references in command instructions:
...
| `Glob` | `glob` |
| `Grep` | `grep` |
| `EnterPlanMode` | *(not available)* |

### Per-Command Adaptations

**`/plan_w_team`**

- Remove `disallowed-tools: Task, EnterPlanMode` from frontmatter. Instead, set `agent: planner` and define a planner agent with `tools: { task: false }`.
- Replace all references to `TaskCreate`, `TaskUpdate`, `TaskList`, `TaskGet` with `todowrite` / `todoread` equivalents. The plan format's `## Team Orchestration` section should reference `todowrite`/`todoread` + `task` instead of `TaskCreate`/`TaskUpdate` + `Task`.
- Remove `EnterPlanMode` references — OpenCode has no equivalent. The plan command's tool restriction is sufficient.
- Update the orchestration reference path mention to note the OpenCode tool mapping table.

**`/build`**

- Replace all `TaskCreate` → `todowrite`, `TaskUpdate` → `todowrite`, `TaskList` → `todoread`, `TaskGet` → `todoread`, `TaskOutput` → `task` references.
- Replace `Task({ resume: agentId, ... })` with `task` resume equivalent.
- The error recovery section's `Task(...)` calls become `task(...)`.

**`/review`**

- Replace `run_in_background: true` with OpenCode's equivalent for background task execution.
- Replace `TaskOutput` with the OpenCode mechanism for collecting background task results.

**`/dev`**

- Remove `argument-hint` from frontmatter. Add `subtask: true` if the dev server should run as a subtask.
...
- Remove `argument-hint` from frontmatter.
- Tool name references are minimal in this command — mainly `Bash` → `bash`.

**`/audit-docs`**

- Remove `argument-hint` from frontmatter.
- The shell script invocation (`bash {{discovery.agent_dir}}/hooks/audit-docs.sh ...`) works identically on both platforms — the script itself is platform-agnostic.
